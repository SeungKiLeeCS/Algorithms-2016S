...
def data_to_array(data, channels):
    return (np.frombuffer(data, dtype=np.int16).reshape((-1, channels)).astype(float) / 2**15)

#Utilize PyAudio for retrieving data from Sound Card
class SoundCardDataSource(object):
    def __init__(self, num_chunks, channels=1, sampling_rate=44100,chunk_size=1024):
        ...
        if not self.pyaudio.is_format_supported(rate=sampling_rate,input_device=dev['index'],input_channels=channels,input_format=pyaudio.paInt16):
        ...
        # Start the stream
        self.stream = self.pyaudio.open(format=pyaudio.paInt16,channels=self.channels,frames_per_buffer=self.chunk_size,rate=self.fs,
    ...
    def get_buffer(self):
        ...
        return np.concatenate((b, a), axis=0) \
                 .reshape((self.buffer.shape[0] * self.buffer.shape[1],self.buffer.shape[2]))
